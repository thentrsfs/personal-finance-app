<template>
  <q-dialog v-model="potsStore.deletePotDialog">
    <q-card
      :class="$q.screen.lt.sm ? 'q-px-xs q-pt-xs' : 'q-px-md q-pt-md'"
    ...
    >
      <q-card-section class="flex justify-between items-center">
        <div
          ...
          "
        >
          Delete '{{ potsStore.selectedPot.label }}'
        </div>
      ...
      <q-card-section class="column q-pt-md">
        <AddButton
          class="btn-primary"
          @click="potsStore.deletePot"
          :ripple="false"
          color="negative"
          label="Yes, Confirm Deletion"
        />
        <div class="text-center" v-close-popup>
          <AddButton
            class="btn-primary"
            :ripple="false"
            text-color="grey-500"
